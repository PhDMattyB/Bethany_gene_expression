...
# amb vs geo Divergence at 18 degrees -------------------------------------------------

brain_eco18 = read_csv('Brain_eco_div_18.csv')%>% 
  filter(adj.P.Val <= 0.05) %>% 
  mutate(status = 'Outlier') %>% 
  left_join(., 
            brain_count_limma, 
...
row.names(brain_eco18) = brain_eco18$GeneID

eco18_cor_mat = cor(t(brain_eco18[,-1]))

number_comparisons = ncol(brain_eco18) - 1

eco18_cor_mat_upper = eco18_cor_mat
eco18_cor_mat_upper[lower.tri(eco18_cor_mat_upper)] <- NA

eco18_edge_table = eco18_cor_mat_upper %>% 
  as.data.frame() %>% 
  mutate(from = row.names(eco18_cor_mat)) %>% 
  pivot_longer(cols = !from, 
               names_to = 'to', 
               values_to = 'r') %>% 
  filter(is.na(r) == F) %>% 
...
row.names(brain_amb_hyb_18) = brain_amb_hyb_18$GeneID

amb_hyb_18_cor_mat = cor(t(brain_amb_hyb_18[,-1]))

number_comparisons = ncol(brain_amb_hyb_18) - 1

amb_hyb_18_cor_mat_upper = amb_hyb_18_cor_mat
amb_hyb_18_cor_mat_upper[lower.tri(amb_hyb_18_cor_mat_upper)] <- NA

amb_hyb_18_edge_table = amb_hyb_18_cor_mat_upper %>% 
  as.data.frame() %>% 
  mutate(from = row.names(amb_hyb_18_cor_mat)) %>% 
  pivot_longer(cols = !from, 
               names_to = 'to', 
               values_to = 'r') %>% 
  filter(is.na(r) == F) %>% 
...
amb_hyb_18_node_tab = data.frame(
  GeneID = c(amb_hyb_18_edge_table_select$from, 
             amb_hyb_18_edge_table_select$to) %>% 
    unique()
) %>% 
  left_join(annotation_ensemble_genes, 
            by = c('GeneID')) %>% 
...
amb_hyb_18_network = graph_from_data_frame(
  amb_hyb_18_edge_table_select,
  vertices = amb_hyb_18_node_tab,
  directed = F
)
...
amb_hyb_18_network_modules <- data.frame(
  GeneID = names(membership(amb_hyb_18_modules)),
  module = as.vector(membership(amb_hyb_18_modules)) 
) %>% 
...
amb_hyb_18_long = brain_amb_hyb_18 %>% 
  ...
  unite(col = ecotemp, 
        ...
        sep = '_',
        remove = F)
amb_hyb_18_high_var_modules = amb_hyb_18_long %>% 
  inner_join(amb_hyb_18_network_modules,
             by = 'GeneID')
...
amb_hyb_18_module_peak_exp = amb_hyb_18_modules_mean_exp %>% 
  group_by(module) %>%
  slice_max(order_by = mean_exp, n = 1) 
# 
# amb_hyb_18_high_var_modules %>% 
#   # filter(module == 5 | module == 6) %>%
#   ggplot(aes(x = ecotemp, y = value)) +
#   geom_line(aes(group = GeneID), alpha = 0.3, color = "grey70") +
#   geom_line(data = amb_hyb_18_modules_mean_exp,  
#             # filter(module == 5 | module == 6), 
#             aes(x = ecotemp, 
#                 y = mean_exp, 
#                 group = module), 
...
dim(amb_hyb_18_subnetwork_nodes)


amb_hyb_18_subnetwork <- graph_from_data_frame(amb_hyb_18_subnetwork_edges,
                                               vertices = amb_hyb_18_subnetwork_nodes,
                                               directed = T)



amb_hyb_18_divergence_gene_network = amb_hyb_18_subnetwork %>% 
  # ggraph()+
  ggraph(layout = "linear",
         circular = T) +
  # geom_edge_link(aes(color = factor(module_rename))) + 
...
row.names(brain_geo_hyb_18) = brain_geo_hyb_18$GeneID

geo_hyb_18_cor_mat = cor(t(brain_geo_hyb_18[,-1]))

number_comparisons = ncol(brain_geo_hyb_18) - 1

geo_hyb_18_cor_mat_upper = geo_hyb_18_cor_mat
geo_hyb_18_cor_mat_upper[lower.tri(geo_hyb_18_cor_mat_upper)] <- NA

geo_hyb_18_edge_table = geo_hyb_18_cor_mat_upper %>% 
  as.data.frame() %>% 
  mutate(from = row.names(geo_hyb_18_cor_mat)) %>% 
  pivot_longer(cols = !from, 
               names_to = 'to', 
               values_to = 'r') %>% 
  filter(is.na(r) == F) %>% 
...
geo_hyb_18_node_tab = data.frame(
  GeneID = c(geo_hyb_18_edge_table_select$from, 
             geo_hyb_18_edge_table_select$to) %>% 
    unique()
) %>% 
  left_join(annotation_ensemble_genes, 
            by = c('GeneID')) %>% 
...
geo_hyb_18_network = graph_from_data_frame(
  geo_hyb_18_edge_table_select,
  vertices = geo_hyb_18_node_tab,
  directed = F
)
...
geo_hyb_18_optimize_num_module <- geo_hyb_18_optimization %>% 
  ggplot(aes(x = resolution, y = num_module)) +
  geom_line(size = 1.1, alpha = 0.8, color = "dodgerblue2") +
  geom_point(size = 3, alpha = 0.7) +
  geom_vline(xintercept = 1, size = 1, linetype = 4) +
...
geo_hyb_18_optimize_num_gene = geo_hyb_18_optimization %>% 
  ggplot(aes(x = resolution, y = num_contained_gene)) +
  geom_line(size = 1.1, alpha = 0.8, color = "violetred2") +
  geom_point(size = 3, alpha = 0.7) +
  geom_vline(xintercept = 1, size = 1, linetype = 4) +
...
wrap_plots(geo_hyb_18_optimize_num_module, 
           geo_hyb_18_optimize_num_gene, nrow = 2)
...
geo_hyb_18_network_modules <- data.frame(
  GeneID = names(membership(geo_hyb_18_modules)),
  module = as.vector(membership(geo_hyb_18_modules)) 
) %>% 
  inner_join(geo_hyb_18_node_tab, by = "GeneID") %>% 
  mutate(NetworkID = 'Geo_vs_Hyb_18')

geo_hyb_18_network_modules %>% 
  group_by(module) %>% 
  count() %>% 
  arrange(-n) %>% 
  filter(n >= 5)
...
geo_hyb_18_network_modules %>% 
  ...
  ungroup() %>% 
  summarise(sum = sum(n))
geo_hyb_18_modules_greater_3 <- geo_hyb_18_network_modules %>%
  group_by(module) %>%
  count() %>%
  arrange(-n) %>%
  filter(n >= 5)
...
geo_hyb_18_long = brain_geo_hyb_18 %>% 
  ...
  unite(col = ecotemp, 
        ...
        sep = '_',
        remove = F)
geo_hyb_18_high_var_modules = geo_hyb_18_long %>% 
  inner_join(geo_hyb_18_network_modules,
             by = 'GeneID')
...
geo_hyb_18_module_peak_exp = geo_hyb_18_modules_mean_exp %>% 
  group_by(module) %>%
  slice_max(order_by = mean_exp, n = 1) 
# 
# geo_hyb_18_high_var_modules %>% 
#   # filter(module == 5 | module == 6) %>%
#   ggplot(aes(x = ecotemp, y = value)) +
#   geom_line(aes(group = GeneID), alpha = 0.3, color = "grey70") +
#   geom_line(data = geo_hyb_18_modules_mean_exp,  
#             # filter(module == 5 | module == 6), 
#             aes(x = ecotemp, 
#                 y = mean_exp, 
#                 group = module), 
...
dim(geo_hyb_18_subnetwork_nodes)


geo_hyb_18_subnetwork <- graph_from_data_frame(geo_hyb_18_subnetwork_edges,
                                               vertices = geo_hyb_18_subnetwork_nodes,
                                               directed = T)



geo_hyb_18_divergence_gene_network = geo_hyb_18_subnetwork %>% 
  # ggraph()+
  ggraph(layout = "linear",
         circular = T) +
  # geom_edge_link(aes(color = factor(module_rename))) + 
...
row.names(Trans_amb_hyb_12) = Trans_amb_hyb_12$GeneID

amb_hyb_12_cor_mat = cor(t(Trans_amb_hyb_12[,-1]))

number_comparisons = ncol(Trans_amb_hyb_12) - 1
...
amb_hyb_12_network_modules <- amb_hyb_12_network_modules %>%
  filter(module %in% amb_hyb_12_modules_greater_3$module)
amb_hyb_12_long = Trans_amb_hyb_12 %>% 
  pivot_longer(!GeneID) %>% 
  as_tibble() %>% 
  # slice(-1) %>% 
  separate(col = name, 
...
# Transgressive @ 12 degrees - geo vs hyb ---------------------------------
...
row.names(Trans_geo_hyb_12) = Trans_geo_hyb_12$GeneID

geo_hyb_12_cor_mat = cor(t(Trans_geo_hyb_12[,-1]))

number_comparisons = ncol(Trans_geo_hyb_12) - 1
...
geo_hyb_12_network_modules <- geo_hyb_12_network_modules %>%
  filter(module %in% geo_hyb_12_modules_greater_3$module)
geo_hyb_12_long = Trans_geo_hyb_12 %>% 
  pivot_longer(!GeneID) %>% 
  as_tibble() %>% 
  # slice(-1) %>% 
  separate(col = name, 
...
# Transgressive exp @ 18 - amb vs hyb -------------------------------------
...
row.names(Trans_amb_hyb_18) = Trans_amb_hyb_18$GeneID

amb_hyb_18_cor_mat = cor(t(Trans_amb_hyb_18[,-1]))

number_comparisons = ncol(Trans_amb_hyb_18) - 1
...
amb_hyb_18_network_modules <- amb_hyb_18_network_modules %>%
  filter(module %in% amb_hyb_18_modules_greater_3$module)
amb_hyb_18_long = Trans_amb_hyb_18 %>% 
  pivot_longer(!GeneID) %>% 
  as_tibble() %>% 
  # slice(-1) %>% 
  separate(col = name, 
...
# Transgressive exp @ 18 - geo vs hyb -------------------------------------
...
row.names(Trans_geo_hyb_18) = Trans_geo_hyb_18$GeneID

geo_hyb_18_cor_mat = cor(t(Trans_geo_hyb_18[,-1]))

number_comparisons = ncol(Trans_geo_hyb_18) - 1
...
geo_hyb_18_network_modules <- geo_hyb_18_network_modules %>%
  filter(module %in% geo_hyb_18_modules_greater_3$module)
geo_hyb_18_long = Trans_geo_hyb_18 %>% 
  pivot_longer(!GeneID) %>% 
  as_tibble() %>% 
  # slice(-1) %>% 
  separate(col = name, 
...
## Pure vs hyb @ 12 degree divergence
div_pure_hyb_12_GO = read_csv('Cyto_trans_pure_hyb_12_intersection_zebrafish.csv') %>% 
  ...
  rename(Network_ID = rowid) %>% 
  # you can apply the previous to each row using map
  mutate(split = map(intersecting_genes, ~ str_split(.x, "\\|")[[1]])) %>% 
  # then unnest the column before further data prep
  unnest(split) %>% 
  select(source, 
         GO_term,
...
## Pure vs hyb @ 18 degrees divergence
...
# TRANS pure vs hyb 12 intersection ---------------------------------------

Trans_12 <- createIgraphFromNetwork("Trans pure vs hyb 12 - intersection")

Trans12_Graph = Trans_12 %>% 
  # ggraph()+
  ggraph(layout = "linear",
         circular = T) +
  # geom_edge_link(aes(color = factor(module_rename))) + 
...
ggsave('Transgressive_expression_12_pure_vs_hyb_network.svg', 
       plot = Trans12_Graph, 
       dpi = 'retina', 
       units = 'cm', 
       width = 100, 
...
# TRANS pure vs hyb 18 intersection ---------------------------------------

Trans_18 <- createIgraphFromNetwork("Trans pure vs hyb 18 - intersection")

Trans18_Graph = Trans_18 %>% 
  # ggraph()+
  ggraph(layout = "linear",
         circular = T) +
  # geom_edge_link(aes(color = factor(module_rename))) + 
...
ggsave('Transgressive_expression_18_pure_vs_hyb_network.svg', 
       plot = Trans18_Graph, 
       dpi = 'retina', 
       units = 'cm', 
       width = 100, 
